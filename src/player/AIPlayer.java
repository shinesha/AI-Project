 ...
 * a random one if random code implmented or used the evaluation function if 
 * evluation code implemented. 
 */
public class AIPlayer implements Player {
	...
	public Move move(Backgammon backgammon) {
		...
		moves = AIPlayer.selectBoard(backgammon, moves);
		moves[indexOfMaxValue]= AIPlayer.bestMove(moves);
		
		AIPlayer.selectBoard(backgammon, moves);
	...
	public static Move[] selectBoard(Backgammon backgammon, Move[] moves) {
		...
		if(moves.length>0){
			for (int i=0; i <moves.length; i++){
				...
				double rnorm = EvalFuncTest2.EvalPercentage(moves[i].getCurrentBoard());
